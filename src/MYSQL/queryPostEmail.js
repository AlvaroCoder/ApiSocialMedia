...
const pool = mysql2.createPool(keys);
...
const GET_POSTS_FROM_USER = "SELECT author.nombre, postemail.idPost, postemail.title, postemail.fechaCreacion, postemail.link, postemail.fromUser, postemail.toUser, postemail.subjectEmail, postemail.time2send, postemail.estado FROM author INNER JOIN postemail ON author.idauthor = postemail.idAuthor WHERE author.nombre =  ?;"
const GET_CONTENT_BY_IDPOST= "SELECT * FROM contentEmail WHERE idPost = ?";
const GET_IDAUTHOR_NAME = "SELECT idauthor FROM author WHERE nombre = ?;";
const GET_IDPOST_EMAIL = "SELECT idPost FROM postemail WHERE title = ?;";
...
const getContentEmailByIDPost = async idPost=>{
    const content = await pool.query(GET_CONTENT_BY_IDPOST,[idPost]).then((val)=>{
        return val[0]
    });
    return content;
}
...
module.exports = {getPostDetailsFromUser, createPostEmail, createContentEmail, getPostEmailByTitle, getIDPostEmail, getPostsFromUser};